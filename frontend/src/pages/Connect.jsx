...
export default function Connect() {
  ...
  useEffect(() => {
    ...
    if (authStatus === 'success' && user) {
      setAuthSuccess(true)
      setError('')
      // Show success message for a moment, then clear URL params
      setTimeout(() => {
      ...
      // Clear error params after showing
      setTimeout(() => {
        navigate('/connect', { replace: true })
      }, 5000)
    }
      ...
      <div className="relative z-10 container mx-auto px-4 py-6 max-w-4xl">
        ...
        {authSuccess && (
          <div className="max-w-2xl mx-auto mb-6">
            <div className="flex items-center gap-3 p-4 bg-green-900/20 border border-green-500/30 rounded-lg text-green-400 animate-fade-in">
              <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              </svg>
              <div>
                <p className="font-semibold">🎉 Successfully connected to GitHub!</p>
                <p className="text-sm text-green-300">You can now connect repositories with full access. Use the URL tab below to add a specific repository.</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-slate-900/60 backdrop-blur border border-slate-700/50 rounded-xl p-6 shadow-xl">
              <div className="text-center mb-5">
                ...
                <p className="text-slate-400 text-sm mb-4">
                  {authSuccess 
                    ? "✅ Connected! Now you can add repositories below." 
              ...
              <div className="space-y-2.5 mb-5">
                <div className="flex items-center gap-2.5 text-slate-300 text-sm">
                  <div className={`w-1.5 h-1.5 rounded-full ${authSuccess ? 'bg-green-400' : 'bg-green-400'}`}></div>
                  <span>Access all public repositories</span>
                  {authSuccess && <span className="text-green-400 text-xs">✓</span>}
                </div>
                <div className="flex items-center gap-2.5 text-slate-300 text-sm">
                  <div className={`w-1.5 h-1.5 rounded-full ${authSuccess ? 'bg-green-400' : 'bg-green-400'}`}></div>
              ...
              <button
                ...
                className={`group w-full py-3 px-5 rounded-lg text-white font-semibold transition-all duration-300 shadow-lg hover:scale-[1.01] ${
                  authSuccess 
                    ? 'bg-green-600 cursor-default' 
                    : 'bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-700 hover:to-blue-700 hover:shadow-violet-500/20'
                }`}
              >
